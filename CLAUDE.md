...
- `QGC_商標客製化_工作報告_2026-07-21.md` — `custom/` 白牌客製化建置架構工作報告，對應第 8 節
...
- [x] **`custom/` 客製化建置架構、換品牌圖示** — 2026-07-21 已完成（使用者提前於雲台核心功能之前提出並要求優先處理），見第 8 節與 `QGC_商標客製化_工作報告_2026-07-21.md`。Windows 圖示/桌面捷徑/工作列圖示、App 內 logo 已驗證生效；macOS/Linux/Android 只做了圖示檔案與 CMake 路徑覆蓋，未實機建置驗證
- [ ] Android 版本（AirLink 3）——`custom/android/res/drawable-*/icon.png` 圖示已就緒，但 Android 建置本身（AndroidManifest、簽章、Qt for Android 環境等）仍完全未開始

## 8. 商標客製化（White-Label Build）

這部分跟 AVIX 雲台功能無關，是獨立的品牌置換任務，走 QGC 官方既有的白牌客製化機制，**不是**本專案自建的東西。

### 機制原理

- 根目錄 `CMakeLists.txt` 在 configure 階段偵測 `IS_DIRECTORY ${CMAKE_SOURCE_DIR}/custom`，只要這個目錄存在就自動開啟 `QGC_CUSTOM_BUILD`、`add_subdirectory(custom)`，不需要額外的建置參數。這是**唯一**的掛載點，`custom/` 以外沒有動任何既有檔案。
- 官方範本在 `custom-example/`（複製成 `custom/` 即生效）。**這個範本內容遠超過品牌置換**——還包含 `CustomFirmwarePlugin`/`CustomAutoPilotPlugin`（換一整套飛控/載具邏輯）、`CustomOptions`（改 UI 行為，例如關掉多載具列表）、`paletteOverride`（換整套色盤）、停用 APM/PX4 Plugin Factory 等。**目前 `custom/` 只精簡實作了品牌置換需要的部分**（`CustomPlugin` 只覆蓋 `brandImageIndoor`/`brandImageOutdoor` + URL 攔截器），**沒有**照抄整份範本。
- 品牌圖示的置換透過兩條路徑：
  1. `custom/cmake/CustomOverrides.cmake` 直接覆蓋 `QGC_WINDOWS_ICON_PATH`/`QGC_MACOS_ICON_PATH`/`QGC_APPIMAGE_ICON_PATH` 這幾個 CMake cache 變數，影響 EXE/桌面捷徑/App Bundle/AppImage 這類「建置期決定」的圖示。
  2. `custom/src/CustomPlugin.cc` 安裝一個 `QQmlAbstractUrlInterceptor`：攔截所有 `qrc:/res/*` 查找，若 `:/Custom/res/*` 底下存在同名檔案就自動轉向。既有程式碼裡寫死的 `qrc:/res/QGCLogoFull.svg`、`:/res/qgroundcontrol.ico`（`QGCApplication.cc:153` 的 `setWindowIcon()`）等引用**完全不用改**，執行期自動吃到客製化版本。

### 若要擴充客製化範圍

**保持精簡原則**，跟第 3 節「能不動就不動」的精神一致：需要什麼 override 就只加什麼，不要一次把 `custom-example/` 整包搬過來（尤其 `CustomFirmwarePlugin`/`CustomOptions` 那些會改變飛控偵測、UI 行為的部分，跟這個 fork 一般連線功能有直接衝突風險，加之前要先跟使用者確認）。

### 刻意排除的圖示

- `resources/QGCLogoArrow.svg`——地圖上「已知航向」時的 GCS 位置標記，會依實際航向旋轉，是功能性方向指示，不是靜態商標，不覆蓋
- `deploy/windows/installheader.bmp`——NSIS 安裝程式歡迎橫幅，只在安裝當下出現一次，非日常會看到的商標，暫未客製化

### 已知限制

- 來源圖檔僅 447×447，macOS icns 512px 與 App 內 SVG 512px 版本屬於向上放大（無透明背景，是實心正方形背景圖），未來若要支援更大尺寸顯示建議索取更高解析度或向量原始檔
- macOS／Linux／Android 三個平台只做了圖示檔案與 CMake 路徑覆蓋，**沒有**在對應平台實機建置驗證過（目前開發環境僅 Windows）
- `custom/res/QGCLogoFull.svg`／`QGCLogoWhite.svg`／`QGCLogoBlack.svg` 三檔內容目前完全相同（來源圖沒有深色/淺色主題變體）
